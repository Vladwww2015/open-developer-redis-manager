@extends('open-developer-redis-manager::layout')
...
<div class="card card-primary">
    ...
    <div class="card-body table-responsive">
        ...
        <table class="table table-hover select-table">
            <thead>
            <tr>
                <th><input type="checkbox" id="grid-select-all" class="key-select-all form-check-input" onchange="developer.grid.select_all(event,this)"></th>
                <th>Key</th>
                <th>Type</th>
                <th>TTL(s)</th>
                <th>Action</th>
            ...
            @foreach($keys as $index => $key)
                <tr data-key="{{$key['key']}}" class="row-{{$key['key']}}">
                    ...
                    <td><span class="badge bg-{{ \OpenDeveloper\Developer\RedisManager\RedisManager::typeColor($key['type']) }}">{{ $key['type'] }}</span></td>
                    <td>{{ $key['ttl'] }}</td>
                    <td>
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        developer.ajax.post(url,data,function(data){
                            console.log(data);
                            developer.toastr.success('Key ' + key + ' deleted');
        ...
        document.querySelector('a.key-delete-multi').addEventListener('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    developer.ajax.post(url,data,function(result){
                        developer.toastr.success(data.key.length + ' keys deleted');
                        developer.ajax.reload();
                    });
                }
            });
